...
def run_xgboost_inference(
    ...
    Args:
        ...
        scaler_path: Path to saved RobustScaler (OPTIONAL - currently not used)
    ...
    Processing Steps:
        ...
        4. Make RUL predictions (NO SCALING - tree models don't require it)
        5. Save predictions with metadata
    Note: XGBoost was trained on raw features without scaling.
          Tree-based models don't require feature normalization.
    """
    print(f"\n{'='*60}")
    ...
    if os.path.exists(scaler_path):
        print(f"\n⚠️  RobustScaler found but NOT applied (XGBoost uses raw features)")
    else:
        print(f"\n✓ No scaler needed - XGBoost trained on raw features")
    ...
    if partition_dirs:
        ...
        for partition_dir in sorted(partition_dirs):
            ...
            print(f"🔮 Running inference on raw features...")
        ...
        print(f"Using {len(feature_cols)} feature columns for prediction")
        X = df_features[feature_cols].values.astype('float32')
        print(f"\n🔮 Running inference on raw features...")
        y_pred = xgb_model.predict(X)
        predictions_df = pd.DataFrame({
            'unit': df_features['unit'],
            'time': df_features['time'],
...
def main(snapshotdate):
    ...
    scaler_path = os.path.join(base_dir, "model_bank/xgboost_feature_scaler.pkl")
