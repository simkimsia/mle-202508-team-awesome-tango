...
def process_gold_label_xgboost(
    ...
    # Create output directory for this snapshot date
    ...
    # Load data (prefer single file, fall back to partitioned)
    ...
    if os.path.exists(single_file_path):
        ...
        # Read partitioned data (dataset=X directories)
        # PyArrow will automatically discover and read all partitions
        print(f"Loading partitioned data from {input_dir}")
        df = pd.read_parquet(input_dir)

    print(f"Loaded {len(df):,} rows")

    # Extract label columns matching notebook format
    # Keep 'unit' (formatted string) for XGBoost feature engineering grouping
    # Keep 'dataset' for potential use as categorical feature
    # Format matches notebook's df_train_xgb structure
