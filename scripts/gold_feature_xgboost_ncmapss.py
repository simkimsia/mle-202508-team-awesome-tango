...
def main(snapshotdate):
    ...
    gold_feature_dir = f"datamart/gold/feature/xgboost/n_cmapss/snapshot_date={snapshot_date_str}"
    ...
    # 2. For each split:
    #    a. Convert features to float32 (memory optimization)
    #    b. For each of 12 features:
    ...
    #    c. Process features in batches (3 at a time) with gc.collect()
    # 3. Save engineered DataFrames:
    #    - train/features.parquet (shape: ~26.9M rows × 101 columns)
    #    - val/features.parquet (shape: ~6.9M rows × 101 columns)
    #    - test/features.parquet (shape: ~7.3M rows × 101 columns)
    #    - oot/features.parquet (shape: ~13.7M rows × 101 columns)
    # 4. Save feature column names:
    #    - feature_columns.txt (list of 89 features for training)
