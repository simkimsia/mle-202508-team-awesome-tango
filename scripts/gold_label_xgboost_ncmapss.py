...
def main(snapshotdate):
    ...
    gold_label_xgboost_dir = "datamart/gold/label/xgboost/n_cmapss"

    print(f"Input directory: {gold_label_base_dir}")
    print(f"Output directory: {gold_label_xgboost_dir}")

    # Process XGBoost labels
    # Extracts RUL_Clipped labels in DataFrame format for this snapshot date
    ...
    data_processing_gold_label_xgboost.process_gold_label_xgboost(
        gold_label_base_dir, gold_label_xgboost_dir, snapshot_date_str
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Gold XGBoost Label extraction for N-CMAPSS"
    )
